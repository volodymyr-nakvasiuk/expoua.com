js.module("jquery.tooltip_online");
...
(function ($) {
	...
	$.fn.extend({
		tooltip: function (settings) {
			...
			return this.each(function () {
				...
				this.alt = "";
			}).mouseover(save).mouseout(hide).click(function(eventObject){
	...
	function save(eventObject, obj) {
		if (!obj) obj = this;
		if ($.tooltip.blocked || obj == current || (!obj.tooltipText && !settings(obj).bodyHandler)) return;
		current = obj;
		title = obj.tooltipText;
		helper.parent.hide();
		if (settings(obj).bodyHandler) {
			helper.title.hide();
			var bodyContent = settings(obj).bodyHandler.call(obj);
			...
			helper.body.show();
		} else if (settings(obj).showBody) {
			var parts = title.split(settings(obj).showBody);
		...
		if (settings(obj).closeButton){
			$(settings(obj).closeButton, helper.body).click(function(){
				if (tID) clearTimeout(tID);
				current = null;
		...
		if (settings(obj).showURL && $(obj).url()) helper.url.html($(obj).url().replace('http://', '')).show();
		else helper.url.hide();
		helper.parent.addClass(settings(obj).extraClass);
		if (settings(obj).fixPNG) helper.parent.fixPNG();

...
{*
{capture name="el"}{strip}
  {include file="common/menuItem.tpl" controller="admin_ep_buyers_entries" text="Баерки"}
  {if isset($session_user_allow.admin_ep_buyers_registrations.index)}
   <li>
   {if $user_params.controller == "admin_ep_buyers_registrations"}
    <a class="leftMenuSubElement" href="{getUrl controller="admin_ep_buyers_registrations" action="list" search="cancellation_approve_status=0"}"><strong>Регистрации</strong></a>
   {else}
    <a class="leftMenuSubElement" href="{getUrl controller="admin_ep_buyers_registrations" action="list" search="cancellation_approve_status=0"}">Регистрации</a>
   {/if}
   </li>
  {/if}
{/strip}{/capture}
    ...
    {if !empty($smarty.capture.el)}
      <div class="leftMenuGroup"><ul><li>"Баерка"</li></ul></div>
...
{capture name="el"}{strip}
  {include file="common/menuItem.tpl" controller="admin_ep_siteusers"   text="Пользователи сайта"      action="list"}
...
{capture name="el"}{strip}
  ...
  {include file="common/menuItem.tpl" controller="admin_system_logcoordinators" text="Журнал работы координаторов"}
  {include file="common/menuItem.tpl" controller="admin_system_fulltext" action="index" text="Состояние поискового индекса"}
{/strip}{/capture}

<h4>База выставочных центров (<a href="{getUrl add=1 del="page" search="longitude=0"}">без координат</a>)</h4>
...
{foreach from=$list.data item="el" name="fe"}
 <TR class="{cycle values="odd,even"}{if $el.longitude == 0} marked{/if}">
  ...
  <TD><a href="{getUrl controller="admin_ep_brandsevents" action="list" search="expocenters_id=`$el.id`"}">{$el.name}</a></TD>

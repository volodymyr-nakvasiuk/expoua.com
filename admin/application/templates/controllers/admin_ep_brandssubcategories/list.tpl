...
{foreach from=$list.data item="el" name="fe"}
 <TR class="{cycle values="odd,even"}">
  ...
  <TD align="center"><INPUT type="checkbox" disabled="disabled" {if $el.active==1} checked="checked"{/if}></TD>
  <TD align="left"><a href="{getUrl controller="admin_ep_brandssubcategories" action="list" parent=$el.id}">{$el.name}</a></TD>
 </TR>
{/foreach}
</TABLE>
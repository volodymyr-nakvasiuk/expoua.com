...
<TR>
 ...
 {include file="common/Lists/headerElementGeneral.tpl" align="center" name="date_modify" descr="Дата"}
 <TH align="center" width="170">Просмотр</TH>
...
{foreach from=$list.data item="el" name="fe"}
 <TR class="{cycle values="odd,even"}">
  ...
  <TD>{$el.name}</TD>
  <TD align="center">{$el.date_modify}</TD>
  <TD align="center">
<a href="{getUrl controller="admin_ep_companies_news" action="list" parent=$el.id}">Новости</a>,
<a href="{getUrl controller="admin_ep_companies_services" action="list" parent=$el.id}">Услуги</a>,
<a href="{getUrl controller="admin_ep_companies_employers" action="list" parent=$el.id}">Сотрудники</a>
  </TD>

<h4>База пользователей компаний</h4>
...
<TR>
  ...
  {include file="common/Lists/headerElementGeneral.tpl" align="center" name="login" descr="Login"}
...
<TR class="{cycle values="odd,even"}">
  ...
  <TD>
    {if $el.photo_exists}
      <img src="http://admin.expopromoter.com/data/images/user_sites/{$el.id}.jpg?salt={$smarty.now} alt="" width="30" />
    {else}
      &mdash;
    {/if}
  </TD>
  <TD>{$el.login}</TD>
  <TD>{$el.fname} {$el.name}</TD>
  <TD>{$el.text_dolgnost}</TD>
